<template>
  <div class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="glass-modal rounded-3xl max-w-4xl w-full shadow-2xl overflow-hidden border border-white/80 max-h-[95vh] flex flex-col">
      <!-- Modal Header (Non-Printable) -->
      <div class="p-4 border-b border-slate-200/80 flex items-center justify-between bg-white/50 backdrop-blur-md">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 rounded-xl bg-purple-900 flex items-center justify-center text-white">
      ...
      <div
        class="p-8 sm:p-12 overflow-y-auto bg-white text-slate-900 space-y-6 text-xs custom-scrollbar relative"
        :style="bgStyle"
      >
        <!-- Header Kop Surat (Shown when no full background template is uploaded) -->
        <div v-if="!baData?.background_image_url" class="border-b-2 border-slate-900 pb-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div v-if="baData?.logo_url" class="w-14 h-14 rounded-lg border bg-white p-1 shadow-xs">
              <img :src="baData.logo_url" alt="Logo" class="w-full h-full object-contain" />
            </div>
            <div>
          ...
          <table class="w-full text-xs">
            <tbody>
              ...
              <tr class="border-b border-slate-200 bg-slate-50/80">
                ...
                <td class="py-2.5 px-4 font-medium">{{ wo.vendor_name || baData.vendor_name }}</td>
              </tr>
              <tr class="border-b border-slate-200">
              ...
              <tr class="border-b border-slate-200 bg-slate-50/80">
                ...
                <td class="py-2.5 px-4 font-medium">{{ wo.location_name }} (Area: {{ wo.area_name || '-' }})</td>
              </tr>
              <tr class="border-b border-slate-200">
                <td class="py-2.5 px-4 font-semibold text-slate-600">PIC Tim Lapangan</td>
                <td class="py-2.5 px-4 font-medium">{{ wo.pic_name }} {{ wo.pic_phone ? `(${wo.pic_phone})` : '' }}</td>
              </tr>
              <tr>
        ...
        <div v-if="baData.footer_image_url" class="rounded-xl overflow-hidden my-4 border border-slate-200">
          <img :src="baData.footer_image_url" alt="Footer Banner" class="w-full h-auto object-contain" />
        </div>
...
const content = computed(() => {
  ...
  return props.baData.content || (props.baData.content_json ? JSON.parse(props.baData.content_json) : null);
});

const wo = computed(() => content.value?.work_order || {});
...
function replaceVariables(text) {
  ...
  return text
    .replace(/\{\{spk_number\}\}/g, wo.value?.spk_number || props.baData?.spk_number || '')
    .replace(/\{\{title\}\}/g, wo.value?.title || props.baData?.work_order_title || '')
    .replace(/\{\{vendor_name\}\}/g, wo.value?.vendor_name || props.baData?.vendor_name || '')
    .replace(/\{\{location_name\}\}/g, wo.value?.location_name || '')
...
const formattedHeader = computed(() => {
  const raw = props.baData?.header_html || `Pada hari ini <strong>{{ba_date}}</strong>, telah dilakukan pemeriksaan dan verifikasi lapangan atas pelaksanaan seluruh item pekerjaan untuk <strong>{{title}}</strong> di lokasi <strong>{{location_name}}</strong> dengan rincian sebagai berikut:`;
  return replaceVariables(raw);
});

const formattedBody = computed(() => {
  const raw = props.baData?.body_template || `Berdasarkan hasil pemeriksaan bukti foto digital (Before, Process, After) dan verifikasi teknis di lapangan, kedua belah pihak menyatakan bahwa seluruh item pekerjaan telah <strong>SELESAI 100% SECARA BAIK DAN MEMENUHI SPESIFIKASI MUTU</strong>.<br><br>Mitra Vendor memberikan jaminan masa pemeliharaan (garansi mutu) selama <strong>90 (sembilan puluh) hari kalender</strong> terhitung sejak tanggal penandatanganan Berita Acara ini.`;
  return replaceVariables(raw);
});

const signatoriesList = computed(() => {
  if (!props.baData) return [];
  if (props.baData.signatories_json) {
    try {
      const parsed = typeof props.baData.signatories_json === 'string' ? JSON.parse(props.baData.signatories_json) : props.baData.signatories_json;
  ...
  return [
    {
      ...
      company_name: wo.value?.vendor_name || 'Mitra Vendor',
...
const bgStyle = computed(() => {
  const bgUrl = props.baData?.background_image_url || props.baData?.header_image_url;
  if (!bgUrl) return {};
  return {
