      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-lg font-black text-slate-900 tracking-tight flex items-center gap-2">
            <span>Master Client & Mitra Pemberi Tugas</span>
            <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-brand-100 text-brand-900 font-mono">
              {{ vendors.length }} Entitas
            </span>
          </h2>
          <p class="text-xs text-slate-500 font-medium">Kelola data perusahaan client (Indomaret, Alfamart, Smartfren, Perbankan, dll).</p>
        </div>
      </div>
          ...
          <tbody class="divide-y divide-slate-100/80 text-slate-700">
            ...
            <template v-else-if="vendors.length > 0">
              <tr v-for="item in vendors" :key="item.id" class="hover:bg-brand-50/30 transition-colors">
                ...
                <td class="py-3.5 px-4 font-medium text-slate-800">{{ item.contact_person || '-' }}</td>
                <td class="py-3.5 px-4 font-mono text-[11px] text-slate-600">
                  <div>{{ item.phone || '-' }}</div>
                  <div class="text-slate-400 text-[10px]">{{ item.email || '-' }}</div>
                </td>
            ...
            <template v-else>
              <tr>
                <td colspan="6" class="py-12 text-center text-slate-400 font-medium">Belum ada data client.</td>
              </tr>
            </template>
          </tbody>
        </table>
      ...
      <div class="glass-modal rounded-3xl max-w-md w-full shadow-2xl p-6 space-y-4 text-xs border border-white/80">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-3">
          ...
          <div class="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
            ...
            >
              {{ isEditing ? 'Simpan Perubahan' : 'Simpan Data Client' }}
            </button>
          </div>
        </form>
      </div>
...
async function handleSubmit() {
  try {
    if (isEditing.value) {
      ...
      alert('Data client baru berhasil ditambahkan!');
    }
...
async function handleDelete(item) {
  if (!confirm(`Hapus data client "${item.name}"?`)) return;
  try {
